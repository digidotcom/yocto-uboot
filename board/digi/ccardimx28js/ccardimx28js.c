...
int board_eth_init(bd_t *bis)
{
	...
	/* Reset PHY */
	gpio_direction_output(CONFIG_ENET0_PHY_RESET_GPIO, 0);
	udelay(10300);  // from datasheet: tvr + tsr
	gpio_set_value(CONFIG_ENET0_PHY_RESET_GPIO, 1);
...
void fdt_fixup_hwid(void *fdt)
{
	...
	for (i = 0; i < ARRAY_SIZE(propnames); i++) {
		...
		do_fixup_by_path(fdt, "/", propnames[i], str, strlen(str) + 1, 1);
	}
}
