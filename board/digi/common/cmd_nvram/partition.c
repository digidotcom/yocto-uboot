...
extern const char* GetEnvVar(const char* szVar, char bSilent);
...
error:
        printf( "\n" ERROR "Read failed at block @ 0x%08llx\n", ullAddr );
        return 0;
}
...
error:
        printf( "\n" ERROR "Write failed at block @ 0x%08llx\n", ullAddr );
        return 0;
}
...
int PartStrAppendParts( char* szCmdLine, size_t iMaxSize )
{
        ...
        for( i = 0; i < pPartTable->uiEntries; i++ ) {
                ...
                if( uiLastChip != pPart->uiChip ) {
                        ...
                        if( !pPart->uiChip ) {
				const char *mtd_driver = GetEnvVar("mtd_driver", 1);

				if (mtd_driver)
					APPEND_CMDLINE(mtd_driver);
#ifdef CONFIG_MTD_DRIVER_NAME
				else
					APPEND_CMDLINE(CONFIG_MTD_DRIVER_NAME);
#endif
                        }
			else {
                                char szBuffer[ 20 ];
...
int PartSetMtdParts( void )
{
	...
	if (mtd_driver)
		strcat(szTmp, mtd_driver);
#ifdef CONFIG_MTD_DRIVER_NAME
	else
		strcat(szTmp, CONFIG_MTD_DRIVER_NAME);
#endif
