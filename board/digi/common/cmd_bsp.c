...
#endif
	int initrdSize = 0;
	int requires_fdt = 0;	/* DTB image provided as argument */
...
_getpart:
		...
		if (argc >= 4) {
		  ...
		  IS_SATA == pImgSrc->eType) && (argc >= 6)) {
			...
			if (argc >= 8)
				SAFE_STRCAT(szImgFdt, argv[7]);	/* DTB image filename */
        }
	...
	if (NVOS_LINUX == eOSType || NVOS_ANDROID == eOSType) {
		...
		case IS_FLASH:
			...
			break;
		case IS_RAM:
			/* do nothing. FDT may already be in RAM */
			ret = 1;
			break;
		default:
		...
		if (ret) {
			...
			hasfdt = 1;	/* FDT is now available in RAM */
		}
	}

	if (!hasfdt && requires_fdt) {
		printf("A valid Device Tree is required\n");
		goto error;
	}
	...
	if (!hasfdt)
		printf("Booting kernel without Device Tree\n");
...
#if defined(CONFIG_AUTOLOAD_BOOTSCRIPT)
void run_auto_script(void)
{
	...
	/* Save original console flags */
	ulong saved_flags = gd->flags;
	/* Save original netretry */
	char *nretry = getenv("netretry");
	...
	if (s && !strcmp(s, "yes") && bootscript) {
		...
		sprintf( szCmd, "tftpboot 0x%x %s", iLoadAddr, bootscript );
		...
		/* Restore original netretry */
		setenv("netretry", nretry);
...
#else
void run_auto_script(void) {}
#endif /* CONFIG_AUTOLOAD_BOOTSCRIPT */
...
#ifdef CONFIG_MTD_DRIVER_NAME
	ret |= create_dynvar("mtd_driver", CONFIG_MTD_DRIVER_NAME);
#else
	ret |= create_dynvar("mtd_driver", "onboard_boot");
#endif /* CONFIG_MTD_DRIVER_NAME */
