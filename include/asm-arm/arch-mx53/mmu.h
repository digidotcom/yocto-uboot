...
#elif defined(CONFIG_CCIMX53)
	if (virt >= 0x90000000 && virt < (0x90000000 + PHYS_SDRAM_1_SIZE))
		return (unsigned long)((virt - 0x90000000) + PHYS_SDRAM_1);
#if CONFIG_NR_DRAM_BANKS > 1
	if (virt >= 0xD0000000 && virt < (0xD0000000 + PHYS_SDRAM_1_SIZE))
		return (unsigned long)((virt - 0xD0000000) + PHYS_SDRAM_2);
...
void *__ioremap(unsigned long offset, size_t size, unsigned long flags)
{
	if (1 == flags) {
		...
		/**
		 * On the ConnectCore Wi-i.MX53 the memory connected to 2nd CS
		 * is used as uncached/unbufered. This memory is mapped to
		 * 0x90000000 (see mmu mappings in ccimx53.c)
		 */
		if (offset >= PHYS_SDRAM_1 &&
		offset < (unsigned long)(PHYS_SDRAM_1 + PHYS_SDRAM_1_SIZE))
			return (void *)((offset - PHYS_SDRAM_1) + 0x90000000);
		 ...
		 * On the ConnectCore Wi-i.MX53 the memory connected to 2nd CS
		 * is used as uncached/unbufered. This memory is mapped to
		 * 0xD0000000 (see mmu mappings in ccwmx53.c)
		 */
		if (offset >= PHYS_SDRAM_1 &&
		offset < (unsigned long)(PHYS_SDRAM_1 + PHYS_SDRAM_1_SIZE))
			return (void *)((offset - PHYS_SDRAM_1) + 0xd0000000);
#if CONFIG_NR_DRAM_BANKS > 1
		else if (offset >= PHYS_SDRAM_2 &&
			offset < (unsigned long)(PHYS_SDRAM_2 + PHYS_SDRAM_2_SIZE))
			return (void *)((offset - PHYS_SDRAM_2) + 0xD0000000);
#endif /* CONFIG_NR_DRAM_BANKS > 1 */
#endif /* CONFIG_CCIMX5_SDRAM_128MB */
#else
		if ((offset >= PHYS_SDRAM_1) &&
		(offset < (unsigned long)(PHYS_SDRAM_1 + PHYS_SDRAM_1_SIZE)))
