...
extern unsigned int get_platform_loadaddr(char *);
extern unsigned int get_tftp_available_ram(unsigned int loadaddr);
extern int variant_machid(void);
...
#ifdef CONFIG_CMDLINE_MAC_ADDRESSES
			/* Append MAC addresses with the format
			 * 	ethaddrN=XX:XX:XX:XX:XX:XX
			 *  Linux will use these to init the MAC on the network
			 *  interfaces instead of depending on U-Boot or needing to
			 *  read them from the NVRAM.
			 */
			if((vp = GetEnvVar("ethaddr", 1 )) != NULL) {
				if (is_valid_ether_addr((const u8 *)vp)) {
					SAFE_STRCAT(szBootargs, " ethaddr1=");
					SAFE_STRCAT(szBootargs, vp);
				}
			}
			if((vp = GetEnvVar("wlanaddr", 1 )) != NULL) {
				if (is_valid_ether_addr((const u8 *)vp)) {
					SAFE_STRCAT(szBootargs, " ethaddr2=");
					SAFE_STRCAT(szBootargs, vp);
				}
			}
			if((vp = GetEnvVar("eth1addr", 1 )) != NULL) {
				if (is_valid_ether_addr((const u8 *)vp)) {
					SAFE_STRCAT(szBootargs, " ethaddr3=");
					SAFE_STRCAT(szBootargs, vp);
				}
			}
			if((vp = GetEnvVar("btaddr", 1 )) != NULL) {
				if (is_valid_ether_addr((const u8 *)vp)) {
					SAFE_STRCAT(szBootargs, " btaddr1=");
					SAFE_STRCAT(szBootargs, vp);
				}
			}
#endif

			/* Append wdmode if it exists */
			if((vp = GetEnvVar("wdmode", 1 )) != NULL) {
				SAFE_STRCAT(szBootargs, " wdmode=");
				SAFE_STRCAT(szBootargs, vp);
			}
...
int generate_dynamic_vars(void)
{
	...
	ret |= create_dynvar(SNFS, "root=nfs nfsroot=${serverip}:");
#ifdef CONFIG_UBOOT_SPLASH
...
#endif
#ifdef CONFIG_MODULE_NAME_WCE
	/* Calculate wceloadaddr basing on available RAM */
	sprintf(temp, "0x%x", get_platform_loadaddr("wceloadaddr"));
	ret |= create_dynvar("wceloadaddr", temp);
...
int bsp_init(void)
{
	...
	gd->bd->bi_arch_number = variant_machid();
#endif
