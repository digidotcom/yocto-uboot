...
#if defined(CONFIG_NO_BATTERY) || defined(CONFIG_FLOATING_BATT_PIN)
	clrsetbits_le32(&power_regs->hw_power_dcdc4p2,
...
static void mxs_init_batt_bo(void)
{
	...
	clrsetbits_le32(&power_regs->hw_power_battmonitor,
		...
		CONFIG_BATT_BO_STEPS << POWER_BATTMONITOR_BRWNOUT_LVL_OFFSET);
				...
				uint32_t new_target, uint32_t new_brownout)
{
	...
	if ((new_brownout << cfg->bo_offset_offset) > cfg->bo_offset_mask)
		new_brownout = cfg->bo_offset_mask >> cfg->bo_offset_offset;
...
void mxs_power_init(void)
{
	...
	mxs_power_set_vddx(&mxs_vddio_cfg, 3300, CONFIG_VDDIO_BO_VOLTAGE);
	mxs_power_set_vddx(&mxs_vddd_cfg, VDDD(CONFIG_CPU_FREQ), VDDD_BO_VOLTAGE);
