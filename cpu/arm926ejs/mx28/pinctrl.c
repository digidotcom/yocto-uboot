...
/* Reconfigure pins as GPIO/INPUT */
void pin_unset_group(struct pin_group *pin_group)
{
	u32 p;
	struct pin_desc *pin;
	...
	for (p = 0; p < pin_group->nr_pins; p++) {
		pin = &pin_group->pins[p];
		/* Set as inputs, GPIO */
		pin_gpio_direction(pin->id, 0);
		pin_set_type(pin->id, PIN_GPIO);
		/* reset values for voltage, strength and pullup */
		pin_set_strength(pin->id, PAD_4MA);
		pin_set_voltage(pin->id, PAD_3V3);
		pin_set_pullup(pin->id, 0);
	}
}
