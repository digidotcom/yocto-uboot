...
u32 get_fpm_clk(void)
{
	if (__REG(MXC_CCM_CCR) & MXC_CCM_CCR_FPM_MULT_MASK)
		return (CLK_CKIL_FREQ * 1024);
	else
		return (CLK_CKIL_FREQ * 512);
}
...
u32 get_lp_apm(void)
{
	...
	if (__REG(MXC_CCM_CCSR) & MXC_CCM_CCSR_LP_APM_SEL)
		ret_val = get_fpm_clk();
	else
...
static u32 __get_ipg_clk(void)
{
	...
	ipg_podf = __REG(MXC_CCM_CBCDR);
	ipg_podf = (ipg_podf & MXC_CCM_CBCDR_IPG_PODF_MASK) >>
			MXC_CCM_CBCDR_IPG_PODF_OFFSET;
	return __get_ahb_clk() / (ipg_podf + 1);
}
...
static u32 __get_nfc_clock(void)
{
	u32 reg, nfc_podf;
	reg = __REG(MXC_CCM_CBCDR);

	nfc_podf = (reg & MXC_CCM_CBCDR_NFC_PODF_MASK) >>
			MXC_CCM_CBCDR_NFC_PODF_OFFSET;

	return 	__get_emi_slow_clk() / (nfc_podf + 1);
}
...
unsigned int mxc_get_clock(enum mxc_clock clk)
{
	...
	case MXC_NFC_CLK:
		return __get_nfc_clock();
	case MXC_CSPI_CLK:
		return __get_cspi_clk();
	case MXC_FEC_CLK:
		return __decode_pll(PLL1_CLK, CONFIG_MX51_HCLK_FREQ);
	case MXC_ESDHC_CLK:
		return __decode_pll(PLL3_CLK, CONFIG_MX51_HCLK_FREQ) / 4;	/* FIXME: PPH revisit this */
	default:
		break;
	}
	return -1;
...
void setup_pll1_freq_800mhz(void)
{
	unsigned int reg;
	int i;

	/* Set PLL1 @800MHz
	 * Most of the following code has been taken from linux (clock.c)
	 * Change the PLL1 to the requested frequency. First, move the DDR
	 * to a different clock, then move the ARM to step clock, change
	 * the PLL and restore everyghing back.
	 */
	reg = readl(MXC_CCM_CBCDR);
	reg &= ~MXC_CCM_CBCDR_DDR_HF_SEL;
	writel(reg, MXC_CCM_CBCDR);

	/* Wait until handsake is completed */
	i = 10000000;
	while (i--) {
		reg = readl(MXC_CCM_CDHIPR);
		if (!(reg & MXC_CCM_CDHIPR_DDR_HF_CLK_SEL_BUSY))
			break;
	}
	if (i == 0)
		puts("### ERROR: changing DDR clock\n");

	/* Change the source of pll1_sw_clk to be the step_clk */
	reg = readl(MXC_CCM_CCSR);
	reg |= MXC_CCM_CCSR_PLL1_SW_CLK_SEL;
	writel(reg, MXC_CCM_CCSR);

	/* Stop the PLL */
	reg = readl(PLL1_BASE_ADDR + MXC_PLL_DP_CTL);
	reg &= ~MXC_PLL_DP_CTL_UPEN;
	writel(reg, PLL1_BASE_ADDR + MXC_PLL_DP_CTL);

	/* Clear AREN bit in PLL1 (to avoid restart during MFN change) */
	reg = readl(PLL1_BASE_ADDR + MXC_PLL_DP_CONFIG);
	reg &= ~MXC_PLL_DP_CONFIG_AREN;
	writel(reg, PLL1_BASE_ADDR + MXC_PLL_DP_CONFIG);

	/* PDF and MFI */
	writel(DP_OP_800, PLL1_BASE_ADDR + MXC_PLL_DP_OP);
	/* PLL1 workaround: MFN/(MFD+1) = 180/(179+1) = 1
	 * These values come from Freescale's patch ENGR00151966
	 * that we apply here instead of in lowlevel_init.S.
	 * The reason is Digi modules lowlevel init start the PLL
	 * at 800MHz and the PLL is set to 800MHz in this function.
	 * The PLL is reset and enabled with these values and later
	 * the MFN is set to a value of 60 to accurately get the
	 * 800MHz value. */
	/* MFD */
	writel(179, PLL1_BASE_ADDR + MXC_PLL_DP_MFD);
	/* MFN */
	writel(180, PLL1_BASE_ADDR + MXC_PLL_DP_MFN);

	reg = readl(PLL1_BASE_ADDR + MXC_PLL_DP_CTL);
	/* Set the PLM (phase lock mode) bits */
	...
	if (i == 0)
		puts("### ERROR: PLL1 relock failed\n");

	/* Reprogram the PLL1 to the next settings:
	 * MFN = 60 -> 60/(179+1) = 1/3 -> results in 800 MHz */
	writel(60, PLL1_BASE_ADDR + MXC_PLL_DP_MFN);

	/* Set LDREQ to notify MFN change */
	reg = readl(PLL1_BASE_ADDR + MXC_PLL_DP_CONFIG);
	reg |= MXC_PLL_DP_CONFIG_LDREQ;
	writel(reg, PLL1_BASE_ADDR + MXC_PLL_DP_CONFIG);
	/* Wait till LDREQ is cleared */
	i = 10000000;
	while (i--) {
		reg = readl(PLL1_BASE_ADDR + MXC_PLL_DP_CONFIG);
		if (!(reg & MXC_PLL_DP_CONFIG_LDREQ))
			break;
	}
	if (i == 0)
		puts("### ERROR: PLL1 MFN update failed\n");
	/* Delay more than 4.6 usec */
	i = 100;
	while (i--) {}
