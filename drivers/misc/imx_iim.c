...
u32 sense_fuse(s32 bank, s32 row, s32 bit)
{
...
int iim_read_hw_id(u8 *data)
{
	s32 bank = CONFIG_IIM_HWID_BANK;
	s32 row  = CONFIG_IIM_HWID_ROW;
...
int iim_blow_func(char *func_name, char *func_val)
{
	...
	418bccd09b53bee1ab59e2662b3c7877bc0094caee201052add49be8780dff95 */
	...
	} else if (0 == strcmp(func_name, "hwid")) {
		u8 hw[6] = { 0 };

		if (NULL == func_val) {
			/* Read the hw id and print it */
			iim_read_hw_id(hw);
		...
		eth_parse_enetaddr(func_val, hw);
		for (i = 0; i < 6; ++i)
			fuse_blow_row(1, i + 9, hw[i]);
	} else {
