...
#ifndef	CONFIG_SATA_RETRY_COUNT
# define CONFIG_SATA_RETRY_COUNT	3	/* # of timeouts before giving up */
#endif

#ifndef CONFIG_SATA_RETRY_TIMEOUT
# define CONFIG_SATA_RETRY_TIMEOUT	3	/* # seconds between retries */
#endif
...
static int ahci_init_one(int pdev)
{
	...
	static struct ahci_probe_ent *prev_probe_ent = NULL;

	/* If already initialized, free up previous probe_ent */
	if (init_done && (prev_probe_ent != NULL)) {
		free(prev_probe_ent);
		prev_probe_ent = NULL;
	}
			...
			u8 port, unsigned char *buf, int buf_len)
{
	...
	struct ahci_sg *ahci_sg =
	    (struct ahci_sg *)ioremap_nocache(iomem_to_phys((ulong)pp->cmd_tbl_sg),
		    0);
#else
...
static void ahci_fill_cmd_slot(struct ahci_ioports *pp, u32 cmd_slot, u32 opts)
{
	...
	struct ahci_cmd_hdr *cmd_hdr =
	    (struct ahci_cmd_hdr *)ioremap_nocache(iomem_to_phys((ulong)(pp->cmd_slot
					...
					u8 port)
{
	...
	pp->cmd_tbl_sg =
		...
		AHCI_CMD_TBL_HDR);
	writel_with_flush(iomem_to_phys((ulong)pp->cmd_slot), &(port_mmio->clb));
...
int init_sata(int dev)
{
	...
	/* try max. CONFIG_SATA_RETRY_COUNT times, in case of no port device detected */
	retry_cnt = 0;
	do {
		if (retry_cnt) {
			printf("\nCouldn't detect port device. Retrying (%d / 3)...\n\n", retry_cnt);

			/* wait CONFIG_SATA_RETRY_TIMEOUT secs between retries */
			for (i = 0; i < CONFIG_SATA_RETRY_TIMEOUT; i++) {
				if( ctrlc() || had_ctrlc() )
					return -3;
		...
		if (ahci_init_one(dev) != 0) {
			printf("Can not init AHCI device\n");
			return -2;
		}

		probe_ent = (struct ahci_probe_ent *)sata_dev_desc[dev].priv;
		linkmap = probe_ent->link_port_map;
	} while ((linkmap == 0) && (retry_cnt++ < CONFIG_SATA_RETRY_COUNT));
...
#define PRINT_SATA_READLOAD(args...)             do{ if (!LoadFileSilent) {printf(args);}} while(0)
...
#ifndef CONFIG_SATA_SILENT_READ
		PRINT_SATA_READLOAD(".");
#endif
	} while (blks != 0);
...
#ifdef CONFIG_ARCH_MMU
	id_orig = (u16 *)malloc(ATA_ID_WORDS * 2);
	if (!id_orig) {
		printf("id malloc failed\n\r");
		return -1;
	}

	id = ioremap_nocache((ulong)id_orig, 0);
#else
