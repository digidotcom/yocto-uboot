...
#define I2C_MAX_TIMEOUT		(100000/4)
...
extern int i2c_verify_stuck_and_recover(void);
...
int i2c_probe(uchar chip)
{
	...
	ret = tx_byte(chip << 1);
	__REG16(I2C_BASE + I2CR) = I2CR_IEN;
	/* Without this delay the audio chip is not detected */
	udelay(100);
...
static int i2c_addr(uchar chip, uint addr, int alen)
{
	...
	if (!i2c_verify_stuck_and_recover())
		i2c_reset();
#endif
