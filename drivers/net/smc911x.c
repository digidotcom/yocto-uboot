...
{
#ifdef CONFIG_SMC911X_SET_MAC_INTERNALLY
...
#if defined(CONFIG_MX53) && !defined(CONFIG_CCIMX53)
	...
	/* Set PME type to push-pull, active low, enabled */
	{
		u32 reg;

		reg = smc911x_reg_read(dev, PMT_CTRL);
		reg &= ~PMT_CTRL_PME_POL;
		reg |= PMT_CTRL_PME_TYPE | PMT_CTRL_PME_EN;
		smc911x_reg_write(dev, PMT_CTRL, reg);
	}
#endif
	return 0;
