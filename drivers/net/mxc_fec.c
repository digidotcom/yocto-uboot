...
#include <asm/io.h>
extern void *iomem_to_phys(unsigned long virt);
...
extern int fec_get_mac_addr(unsigned int index, unsigned char *mac);
#else
...
static inline void fec_localhw_setup(volatile fec_t *fecp)
{
	 ...
	 */
	PUTS_INFO("FEC: enable RMII gasket\n");
...
struct fec_info_s fec_info[] = {
	{
	 ...
	 {
	 1,			/* index */
	 CONFIG_FEC1_IOBASE,	/* io base */
	 CONFIG_FEC1_PHY_ADDR,	/* phy_addr */
	 0,			/* duplex and speed */
	 0,			/* phy name */
	 0,			/* phyname init */
	 0,			/* RX BD */
...
static void mxc_fec_mii_init(struct eth_device *dev)
{
...
#ifdef CONFIG_MX28
	clk = mxc_get_clock(MXC_AHB_CLK);
#else
	clk = mxc_get_clock(MXC_IPG_CLK);
#endif
	/*
	 * Configure MDC freq to 2.5MHz
	 * MDC freq = clk/(MII_SPEED * 2)
	 * MII_SPEED = clk/(MDC freq * 2)
	 */
	mscr = FEC_MSCR_MII_SPEED(clk/5000000);
#ifdef CONFIG_MX28
	/* Configure MDIO hold time of 10ns
	 * holdtime = clk/100MHz */
	mscr |=  FEC_MSCR_HOLDTIME(clk/100000000);
#endif
	fecp->mscr = mscr;
...
static int fec_get_hwaddr(struct eth_device *dev, unsigned char *mac)
{
	...
	return fec_get_mac_addr(info->index, mac);
#else
	return fec_get_mac_addr(mac);
#endif
}
...
//	memcpy(ioremap_nocache((ulong)info->txbd[info->txIdx].cbd_bufaddr,
	memcpy((void *)ioremap_nocache((uint)info->txbd[info->txIdx].cbd_bufaddr,
			length),
			(void *)packet, length);
#else
...
#endif
#ifdef CONFIG_VARIABLE_ARP_TIMEOUT
			arp_tout = CONFIG_STANDARD_ARP_TIMEOUT;
#endif
...
void fec_reset(struct eth_device *dev)
{
	...
	/* Resetting the Ethernet controller means setting the MSCR to zero
	 * thus disabling the MII clock and preventing the MII commands from
	 * working. If the support for MII is enabled, we must init the MII
	 * clock again.
	 */
	mxc_fec_mii_init(dev);
#endif
#ifdef CONFIG_MX28
	/* In MX28, a FEC reset also resets the MAC address registers.
...
int mxc_fec_initialize(bd_t *bis)
{
	...
	for (i = 0; i < sizeof(fec_info) / sizeof(fec_info[0]); i++) {
		...
		if (!fec_info[i].init_only_mac) {
#ifdef CONFIG_ARCH_MMU
			fec_info[i].rxbd =
				(cbd_t *)ioremap_nocache((ulong)iomem_to_phys((ulong)memalign(CONFIG_SYS_CACHELINE_SIZE,
			...
			fec_info[i].txbd =
				(cbd_t *)ioremap_nocache((ulong)iomem_to_phys((ulong)memalign(CONFIG_SYS_CACHELINE_SIZE,
							...
							CONFIG_SYS_CACHELINE_SIZE);
			fec_info[i].txbuf =
				(char *)ioremap_nocache((ulong)iomem_to_phys((ulong)memalign(CONFIG_SYS_CACHELINE_SIZE,
							DBUF_LENGTH)),
							CONFIG_SYS_CACHELINE_SIZE);
			for (j = 0; j < PKTBUFSRX; ++j) {
				fec_info[i].rxbuf[j] =
					(char *)ioremap_nocache((ulong)iomem_to_phys((ulong)memalign(PKTSIZE_ALIGN,
							PKTSIZE)),
							PKTSIZE_ALIGN);
			}
#else
			...
			miiphy_register(dev->name, mxc_fec_mii_read,
							mxc_fec_mii_write);
			/* Initialize and register phy to allow using mii commands */
			mxc_fec_mii_init(dev);
#endif
		} /* endif !init_only_mac */

		/* Read MAC address and set it to the FEC */
