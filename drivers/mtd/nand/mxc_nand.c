...
extern inline int skip_swap_bi(int page_addr);
			     ...
			     int column, int page_addr)
{
	...
	case NAND_CMD_READOOB:
		/* Increase the offset with the page size to ensure we read
		 * the byte from the oob if we call read_byte after this commnad */
		info->col_addr = mtd->writesize + column;
...
#ifdef MXC_NAND_DONT_REWRITE_ERASED_PAGES
			      ...
			      int page)
{
	...
	/**
	 * Read back the data from the page... This is needed due to the
	 * the way the nand controller works, not being able to just write
	 * the spare area block (because the spare block is distributed
	 * across the data+spare blocks in 4 512+16 sections.
	 */
	chip->cmdfunc(mtd, NAND_CMD_READ0, 0, page);

	/**
	 * If the sector was not fully erased, this operation will fail,
	 * because the nand controller will not be able to write the new
	 * ecc value.
	 */
	for (i = 0; i < mtd->oobsize; i++) {
		if (info->oob_buf[i] != 0xff)
			return -EIO;
	}

	return mxc_nand_prog_page(mtd, chip, info->data_buf, page, 0, 1);
}

static int mxc_nand_block_bad(struct mtd_info *mtd, loff_t ofs, int getchip)
{
	struct nand_chip *chip = mtd->priv;
	struct nand_info *info = chip->priv;
	int page, bbpos, res = 0;

	page = (int)(ofs >> chip->page_shift) & chip->pagemask;

	if (unlikely(skip_swap_bi(page)))
		bbpos = mtd->writesize > 512 ?
			NAND_LARGE_BADBLOCK_POS : NAND_SMALL_BADBLOCK_POS;
	else
		bbpos = IS_4K_PAGE_NAND ? MX5X_SWAP_BI_SPARE_BB_POS_4K : MX5X_SWAP_BI_SPARE_BB_POS;

	chip->cmdfunc(mtd, NAND_CMD_READ0, 0, page);
	if (info->oob_buf[bbpos] != 0xff)
		res = 1;
...
static int mxc_nand_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	struct nand_chip *chip = mtd->priv;
	struct nand_info *info = chip->priv;
	uint8_t buf[1] = { 0x0f };	/* Use 0x0f to distinguish the blocks we have marked */
	int block, ret;
	...
	if (chip->options & NAND_USE_FLASH_BBT)
		...
		if (unlikely(skip_swap_bi((ofs >> chip->page_shift) & chip->pagemask)))
			chip->ops.ooboffs = mtd->writesize > 512 ?
				NAND_LARGE_BADBLOCK_POS : NAND_SMALL_BADBLOCK_POS;
		else
			chip->ops.ooboffs = IS_4K_PAGE_NAND ? MX5X_SWAP_BI_SPARE_BB_POS_4K : MX5X_SWAP_BI_SPARE_BB_POS;
