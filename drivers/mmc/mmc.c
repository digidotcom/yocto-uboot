...
#if 1
	/* TODO, this was not working in early integration phases with
	 * the CCWi-i.MX51. Check again if things have improved... */
	do {
		/*
...
#define PRINT_MMC_READLOAD(args...)             do{ if (!LoadFileSilent) {printf(args);}} while(0)
#define MMC_MAX_BULK_TRANSFER			1000
...
static ulong mmc_bread(int dev_num, ulong start, lbaint_t blkcnt, void *dst)
{
	...
	do {
		cur = (blocks_todo > MMC_MAX_BULK_TRANSFER) ? MMC_MAX_BULK_TRANSFER : blocks_todo;
		if (mmc_read_blocks(mmc, dst, start, cur) != cur)
			return 0;
		blocks_todo -= cur;
		start += cur;
		dst += cur * mmc->read_bl_len;
#ifndef CONFIG_MMC_SILENT_READ
		PRINT_MMC_READLOAD(".");
#endif
	} while (blocks_todo > 0);
