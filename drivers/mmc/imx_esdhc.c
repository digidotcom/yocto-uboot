...
/* for slow cards, we increase the delays... */
static int delaymult = 1;

extern int detect_mmc_emmc_ddr_port(struct fsl_esdhc_cfg *cfg);
...
/* Enable uSDHC if the config is defined (only for i.MX50 in SDR mode) */
	...
	/* FIXME: PPH, override this configuration until this is working preoperly */
 ...
 */
	...
	mmc->f_min = 400000;
	/* PPH, note: using here a max clock of 50MHz causes problems with several cards */
	mmc->f_max = MIN(mxc_get_clock(MXC_ESDHC_CLK), 25000000);
